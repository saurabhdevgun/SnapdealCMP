...
public static WebDriver getDriver(String driverType) {
		...
		switch (driverType.toLowerCase()) {
			...
			case "chrome":
				...
				cap = new ChromeDriver();
				break;
			// Firefox capabilities - DEFAULT
			case "firefox":
			default:
				cap = new FirefoxDriver();
				break;
		}
		return cap;
	}
		...
		try {
			...
			switch (actionName.toLowerCase()) {
				...
				case "save":
					...
					break;
				// This code is to written to get handles values for calculation purpose- For example the item picked up from gui is as 1,200.00 or 1.200,00 - 
				// After passing through this code it will get converted into 1200.00
				...
				case "savebyintcomma":
					if(webElement.getText() != null && !webElement.getText().isEmpty()) { 
						...
						//result[1] = webElement.getText();
						String receivedvalue = webElement.getText();
						String breakValue [] = receivedvalue.split("[,|.]");
						String finalstring = "";
						if ( breakValue.length > 1)
						{
							for ( int s = 0 ; s < breakValue.length-1 ; s++)
							{
								finalstring = finalstring.concat(breakValue[s]);
							}
							//System.out.println(finalstring);
							finalstring = finalstring.concat(".");
							finalstring = finalstring.concat(breakValue[breakValue.length-1]);
							result[1] = finalstring;
						}
						else
						{
							result[1] = breakValue[0];
						}
					} 
					else if(webElement.getAttribute("value") != null && !webElement.getAttribute("value").isEmpty()) {
						//result[1] = webElement.getAttribute("value");
						//System.out.println("intttttt here");
						//result[1] = webElement.getText();
						String receivedvalue = webElement.getAttribute("value");
						String breakValue [] = receivedvalue.split("[,|.]");
						//String finalstring = breakValue[0];
						String finalstring = "";
						if ( breakValue.length > 1)
						{
							for ( int s = 0 ; s < breakValue.length-1 ; s++)
					...
					if(webElement.getText() != null && !webElement.getText().isEmpty()) { 
						...
						if ( breakValue.length > 1)
						{
							for ( int s = 0 ; s < breakValue.length ; s++)
							{
								finalstring = finalstring.concat(breakValue[s]);
							}
							//System.out.println(finalstring);
						...
						if ( breakValue.length > 1)
						{
							for ( int s = 1 ; s < breakValue.length ; s++)
							{
								finalstring = finalstring.concat(breakValue[s]);
